...
@st.cache_resource
def load_embeddings():
    """Loads the sentence-transformer model for text vectorization."""
...
RECEIPT_DIR = os.path.join("Agentic RAG", "Images_data")
...
def extract_text_from_receipts() -> List[str]:
    ...
    for file in os.listdir(RECEIPT_DIR):
        if file.lower().endswith((".png", ".jpg", ".jpeg")):
            ...
            img = cv2.imread(path)
            if img is None:
                # Skip files that couldn't be read as images
                continue
...
def retrieval_agent(state: AgentState):
    # If the retriever object follows LangChain's API, use get_relevant_documents.
    # If your retriever exposes a different method (e.g. invoke), adjust accordingly.
    try:
        docs = retriever.get_relevant_documents(state["query"])
    except AttributeError:
        # fallback if retriever has a custom invoke method
...
def analysis_agent(state: AgentState):
    res = llm.invoke(
        analysis_prompt.format(query=state["query"], context=state["context"])
    )
...
uploaded_files = st.file_uploader(
    "Upload receipt images", type=["png", "jpg", "jpeg"], accept_multiple_files=True
)
if uploaded_files:
